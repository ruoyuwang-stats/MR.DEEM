#' Calculate the DEEM estimator and its estimated standard error in the one-sample setting
#'
#' @description
#' This function implements the Debiased Estimating Equation Method (DEEM) for Mendelian
#' randomization in the one-sample setting, producing consistent causal effect estimates even when instruments are weak
#' or correlated.
...
#'
#' @export
est.os.DEEM <- function(index, Gamma_h_lst, gamma_h_lst, gamma_s_lst, Sig_y, Sig_x, Sig_alpha, R_sel) {
  D_x <- list()
  D_y <- list()
  D_alpha <- list()
  ...
  for (i in 1:index) {
    num <- num + t(gamma_h_lst[[i]]) %*% W_or[[i]] %*% r_tilde[[i]] + init * sum(diag(W_or[[i]] %*% D_x[[i]]))
    den <- den + sum(diag(W_or[[i]] %*% D_x[[i]]))
  }
  ...
  for (i in 1:index) {
    num <- num + t(r_tilde[[i]]) %*% W_tau[[i]] %*% r_tilde[[i]] - sum(diag(W_tau[[i]] %*% (D_y[[i]] + (init^2 - 2 * rho_h * init) * D_x[[i]])))
    den <- den + sum(diag(W_tau[[i]] %*% D_alpha[[i]]))
  }

  tau2_h <- c(num / den)  ##decorrelation estimator
  ...
  for (i in 1:index) {
    ...
    D_r.inv[[i]] <- solve(D_y[[i]] + (init^2 - 2 * init * rho_h) * D_x[[i]] + max(tau2_h, 0) * D_alpha[[i]])
    Sig_r[[i]] <- Sig_y[[i]] + (init^2 - 2 * init * rho_h) * Sig_x[[i]] + max(tau2_h, 0) * Sig_alpha[[i]]
  }
  ...
  orEE <- function(beta) {
    g <- 0
    for (i in 1:index) {
      g <- g + t(gamma_h_lst[[i]] - (rho_h - beta) * D_x[[i]] %*% solve(D_y[[i]] + tau2_h * D_alpha[[i]] + (beta^2 - 2 * beta * rho_h) * D_x[[i]]) %*% (Gamma_h_lst[[i]] -
  ...
  for (i in 1:index) {
    M_tmp1 <- cbind(Sig_x[[i]], (rho_h - init) * Sig_x[[i]])
    M_tmp2 <- cbind((rho_h - init) * Sig_x[[i]], Sig_r[[i]])
  ...
  for (i in 1:index) {
    ...
    M_tmp2 <- cbind(W_or[[i]], - (rho_h - init) * (D_r.inv[[i]] %*% D_x[[i]] %*% W_or[[i]] + W_or[[i]] %*% D_x[[i]] %*% D_r.inv[[i]]))
  ...
  for (i in 1:index) {
    ...
    M_tmp2 <- cbind((rho_h - init) * W_tau[[i]] %*% Sig_x[[i]], W_tau[[i]] %*% Sig_r[[i]])
  ...
  den <- rep(0, 2)
  for (i in 1:index) {
    num[1] <- num[1] + sum(diag(W_or[[i]] %*% D_x[[i]] + 2 * init * (rho_h - init) * W_or[[i]] %*% D_x[[i]] %*% D_r.inv[[i]] %*%
                                  D_x[[i]]))
    num[2] <- num[2] + (rho_h - init) * sum(diag(W_or[[i]] %*% D_alpha[[i]] %*% D_r.inv[[i]] %*% D_x[[i]]))
    num[3] <- num[3] + sum(diag(W_tau[[i]] %*% D_x[[i]]))
    den[1] <- den[1] + sum(diag(W_or[[i]] %*% D_x[[i]]))
    den[2] <- den[2] + sum(diag(W_tau[[i]] %*% D_alpha[[i]]))
  }
  a1[3] <- - 1 / den[1] * (num[1] - 2 * init * num[2] * num[3] / den[2])
  a1[1] <- 1

  num <- 0
  den <- 0
  for (i in 1:index) {
    num <- num + t(gamma_h_lst[[i]]) %*% W_or[[i]] %*% gamma_h_lst[[i]] - sum(diag(W_or[[i]] %*% D_x[[i]]))
    den <- den + t(gamma_s_lst[[i]]) %*% W_or[[i]] %*% gamma_h_lst[[i]]
  }
  ...
  den <- 0
  for (i in 1:index) {
    num <- num + (rho_h - init) * sum(diag(W_or[[i]] %*% D_x[[i]] %*% D_r.inv[[i]] %*% D_alpha[[i]]))
    den <- den + sum(diag(W_tau[[i]]%*% D_alpha[[i]]))
  }
